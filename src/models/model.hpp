...
public:
  Model(int height, int width, int numApples, int numSpikes)
      : board(height, width), scoreBoard(height, width), snake(5, 5),
  ...
  void processInput(chtype input) { snake.processInput(input); }
  ...
  void updateState() {
    ...
    if (collidedPiece == nullptr) {
      ...
      if (entities.size() ==
              (uint)(boardDimensions.width * boardDimensions.height) &&
          std::find_if(entities.begin(), entities.end(), [](auto e) {
            return e->getIcon() == 'A';
          }) == entities.end()) {
        game_over = true;
        game_won = true;
  ...
  void addGameOverScreen() const { 
        board.addGameOverScreen(score); 
    }
...
private:
  ...
  const Position getRandomOpenPosition() const {
    ...
    return {
      y : (*it) / boardDimensions.width,
      x : (*it) % boardDimensions.width
    };
  }
